...
export class TranslateFilesService {
  private readonly logger = new Logger(TranslateFilesService.name);
  private keys: string[] = [];
  private readonly languages: Record<string, { [key: string]: string }> = {};
  private declare languagesAvailable: string[];
  private readonly languagesDir = './languages';

  constructor(
    private readonly configService: ConfigService,
    private readonly translateExtractorService: TranslateExtractorService,
  ) {}
  ...
  private async createTranslates() {
  ...
  private async createEntity() {
    const filePath = './libs/backend/entities/src/lib/translate/translate.entity.ts';

    const separator = '\r\n';

    const lines = this.keys.map((key) => `  @ApiProperty()${separator}  declare ${key}: string;${separator}`);
    const fileContent = `import { ApiProperty } from '@nestjs/swagger';${separator}${separator}export class TranslateEntity {${separator}${lines.join(
      separator,
    )}}${separator}`;

    const currentFileContent = await readFile(filePath, { encoding: 'utf8' });

    if (currentFileContent !== fileContent) {
      this.logger.verbose('createFile: translate.entity.ts');
