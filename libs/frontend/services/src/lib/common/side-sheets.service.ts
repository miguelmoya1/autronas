...
export class SideSheetsService extends OverlayService {
  ...
  public override async open<T>(
    ...
    await this.animationService.appearFromRight(this.ref.overlayRef.overlayElement);

    return present;
  }

  protected override async disappear() {
    return this.animationService.disappearToRight(this.ref?.overlayRef.overlayElement);
  }
}
